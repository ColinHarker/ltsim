...
SystemProcess::SystemProcess(std::string t_user, std::string t_pid,
                             std::string t_apu, std::string t_mem,
    ...
    : user(t_user), pid(t_pid), apu(t_apu), mem(t_mem), vsz(t_vsz), rss(t_rss),
      tty(t_tty), stat(t_stat), start(t_start), time(t_time),
      command(t_command){};

void SystemProcess::parse(const char* buffer) { std::cout << buffer << '\n'; }
