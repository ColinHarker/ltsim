...
void displayElement(WindowWrap& disp, int row, int col, string element, flag::printType flag, flag::color c)
{
   switch(flag)
   {
...
void displayPercentColor(WindowWrap& disp, float percent, string str, int row, int col)
{
   if(percent == 0.0)
   {
      displayElement(disp, row, col, str, flag::printType::standard, flag::color::none);
   }
   else if(percent < 60)
   {
      displayElement(disp, row, col, str, flag::printType::use_color, flag::color::green);
   }
   else if(percent >= 60 && percent < 85)
   {
      displayElement(disp, row, col, str, flag::printType::use_color, flag::color::yellow);
   }
   else
   {
      displayElement(disp, row, col, str, flag::printType::use_color, flag::color::red);
   }
}
...
vector<string> parseCommandLineOutput(const char* cmd)
{
   ...
   while(fgets(buffer.data(), buffer.size(), pipe.get()) != nullptr)
   {
      ...
      if((result.find("Filesystem") != string::npos) || (result.find("/dev/loop") != string::npos))
      {
         continue;
      }
